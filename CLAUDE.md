...
Vexy Lines embeds a JSON-RPC 2.0 MCP server on `localhost:47384` (TCP, newline-delimited). A bridge binary (`vexylines-mcp`) converts stdio↔TCP for Claude Desktop/Cursor integration. The Python package provides a `MCPClient` that connects directly via TCP.

**25 tools** organized in five groups:
...
**Fill types:** linear, wave, circular, radial, spiral, scribble, halftone, handmade, fractals, trace

**Coordinates:** All spatial params in pixels at document DPI. Origin is top-left.

### Preference Domain

The app's plist domain changed from `com.vexy-art.lines` (v1.x) to `com.fontlab.vexy-lines` (current). The code in `core/plist.py` has `APP_DOMAIN` constant. Preference keys use middle-dot (`·`, `\u00b7`) as separator on macOS (e.g. `export·dlg·format`).
